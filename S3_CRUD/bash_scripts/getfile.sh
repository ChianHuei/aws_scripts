...
# getfile will have these command line arguments:
# user-name password file-key path-to-save-file-to
# For example: getfile <myName> <myPassword> <'myPhotoOnS3.jpg'> <.\myLocalPhoto.jpg>
...
if [ $numargs -ne 4 ]; then
    echo "usage: user-name password file-key path-to-save-file-to"
    exit
fi
...
S3_CHECK_DOWNLOAD=$(aws s3 cp s3://${BUCKET_USER_DATA}/"${username}"/"${filekey}" "${filepath}" 2>&1)

if [ $? == 0 ]; then
    echo "${filekey} is successfully downloaded."
else
